...
static void ping_prepare_echo( struct icmp_echo_hdr *iecho, u16_t len, int if_checksum)
{
    size_t i;
    ...
    if (if_checksum) {
        ...
        /* If checksum is not used, set it to 0 */
        iecho->chksum = 0;
    }
}

/* Ping using the socket ip */
err_t lwip_ping_send(int s, ip_addr_t *addr, int size, int if_checksum)
{
    int err;
    ...
    ping_prepare_echo(iecho, (u16_t) ping_size, if_checksum);
